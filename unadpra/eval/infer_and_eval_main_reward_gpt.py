...
if __name__ == "__main__":
    ...
    file_name = "test.json"
    save_path = os.path.join("./inference_res/2621", "infer_generate_main_{}_{}_{}".format(args.index, args.stage, file_name))
    with open(save_path, 'r', encoding='utf-8') as f:
    ...
    for index in range(len(infer_data)):
        reference_index= int(infer_data[index]['gpt_ranks'][0])
        # print(reference_index)
        # print(infer_data[index]['suffix'])
        infer_data[index]["infer"]["score"] = rewards[index]
        if 'llama3' in file_name:
            infer_data[index]["infer"]["bleu"] = metrics2.get_bleu(infer_data[index]['infer']['t']['content'], infer_data[index]['suffix'][reference_index-1])


        infer_data[index]["infer"]["bleu"] = metrics2.get_bleu(infer_data[index]['infer']['t'], infer_data[index]['suffix'][reference_index-1])
